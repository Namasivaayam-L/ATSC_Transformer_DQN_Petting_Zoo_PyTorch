...
# Add project root and drl folder to path
...
config = setup_env()
logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s [drl_shared] %(message)s')
logger = logging.getLogger('drl_shared')
...
def train(config):
    ...
    logger.info("Initialized SharedDQN")
    ...
    for ep in range(config['num_episodes']):
        ...
        while not all(terminations.values()):
            ...
            state = next_state
            step += 1
        # episode done: learn from memory
        agent.learn()
