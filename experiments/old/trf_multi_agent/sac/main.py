...
ini_file = "experiments/trf_multi_agent/sac/config.ini"
...
state_shape = env.observation_spaces['1'].shape[0]
action_shape = env.action_spaces['1'].n
experience_replay = memory.Memory(buffer_size, state_shape, action_shape)

for ep in tqdm(range(num_episodes), desc="Running..", unit="epsiode"):
    state, _ = env.reset()
    terminations = {a: False for a in agents}
    while not all(terminations.values()):
        actions = {ts: np.argmax(agents[ts].act(state[ts],batch_size=1)) for ts in env.possible_agents}
        print(actions)
        ...
        if all(terminations.values()):
            for key in agents.keys():
                agents[key].learn(experience_replay.sample(batch_size),batch_size)
