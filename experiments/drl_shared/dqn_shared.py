"""
Shared-parameter DQN variant with NoisyNet exploration, prioritized replay, and n-step returns.
"""
import torch
...
class SharedDQN:
    def __init__(self, num_actions, num_states, width,
                 num_heads, num_enc_layers, embedding_dim,
                 num_bins, batch_size, gamma, learning_rate,
                 buffer_size, n_step, alpha=0.6, beta=0.4,
    ...
    def learn(self):
        ...
        curr = curr.view(self.batch_size, self.memory.n_step, -1)
        nxt = nxt.view(self.batch_size, self.memory.n_step, -1)
        # train
        self.model.train(); self.optimizer.zero_grad()
        q_vals = self.model(curr)
        next_q = self.model(nxt).max(dim=1)[0].detach()
        target = rewards + (self.gamma ** self.n_step) * next_q
        q_a = q_vals.gather(1, actions.view(-1,1)).squeeze()
        loss_per = F.smooth_l1_loss(q_a, target, reduction='none')
